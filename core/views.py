...
class FuelRouteView(APIView):
    def post(self, request):
        ...
        if not api_key:
             return Response({"error": "API Key is missing in .env file"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        headers = {'Authorization': api_key}

        # 3. Helper function to find coordinates
        def get_geo(text):
            url = f"https://api.openrouteservice.org/geocode/search?text={text}"
            try:
                ...
                return data['features'][0]['geometry']['coordinates'], None
            except Exception as e:
                return None, str(e)

        # 4. Get Coordinates for Start and End
        start_coords, error = get_geo(start_location)
        if error: return Response({"error": f"Start Location Error: {error}"}, status=400)

        end_coords, error = get_geo(finish_location)
        if error: return Response({"error": f"Finish Location Error: {error}"}, status=400)

        # 5. Get the Route (The Line on the Map)
        try:
            body = {"coordinates": [start_coords, end_coords]}
            ...
            if route_resp.status_code != 200:
                 return Response({"error": f"Routing Failed: {route_resp.text}"}, status=400)
            ...
            return Response({"error": f"Route Calculation Error: {str(e)}"}, status=500)

        # 6. Fuel Logic (Find stations along the route)
        ...
        for i in range(1, len(coordinates)):
            ...
            # If we have driven 450 miles, look for a gas station
            if current_segment >= 450:
                # Search database for stations near this point (pt2)
                # Optimization: First filter by rough lat/lon box
                candidates = FuelStation.objects.filter(
                    latitude__range=(pt2[1]-1, pt2[1]+1),
                    longitude__range=(pt2[0]-1, pt2[0]+1)
                )
                ...
                for station in candidates:
                    if station.latitude:
                        # Check exact distance (e.g., within 20 miles)
                        d = haversine(pt2[0], pt2[1], station.longitude, station.latitude)
                        if d <= 20:
                            if float(station.retail_price) < best_price:
                                best_price = float(station.retail_price)
                                best_station = station
                
                if best_station:
                    cost = (current_segment / 10) * best_price
                    total_fuel_cost += cost
                    fuel_stops.append({
                        "station": best_station.name,
                        "city": best_station.city,
                    ...
                    current_segment = 0 # Tank is full again

        # Add cost for the remaining miles at the end
        if current_segment > 0:
            last_price = fuel_stops[-1]['price'] if fuel_stops else 3.50
            total_fuel_cost += (current_segment / 10) * last_price

        return Response({
            "route_map_geojson": geometry,
